...
public:
  Read(char *file) {
    ...
    if (graphFile.is_open()) {
      ...
      while (getline(graphFile, lineGraphFile)) {
        ...
        readNodeInformation(node1, node2, edgeWeight, dirigedEdge,
                            lineGraphFile);
  ...
  void readCoordinateTag(float &xCoordinate, float &yCoordinate, N &tag,
                         string line) {
  ...
  void readNodeInformation(N &node1, N &node2, E &edgeWeight, bool &dirigedEdge,
                           string lineGraphFile) {
  ...
  void readGraphInformation(int &numberNodes, bool &dirigedGraph,
                            string lineGraphFile) {
